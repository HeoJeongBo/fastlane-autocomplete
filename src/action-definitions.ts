	...
	upload_to_testflight: {
		...
		parameters: [
			...
			},
			{
				...
				description:
					"If set to true, the `fastlane pilot` command will not wait for the build to be processed",
				required: false,
				defaultValue: "true",
				type: "boolean",
			},
			...
			},
			{
				...
				description:
					"Use version info from uploaded ipa file to determine what build to use for distribution",
				required: false,
				defaultValue: "false",
				type: "boolean",
			},
...
export function generateSnippetForAction(actionDef: ActionDefinition): vscode.SnippetString {
	...
	const parameterStrings = selectedParams
		.map((param, index) => {
